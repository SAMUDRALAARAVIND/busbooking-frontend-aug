...
const SeatLayout = ({ tripId }) => {
  const [isLoading, error, seatData] = useGetSeatData();
...
const useGetSeatData = (refresh) => {
  ...
  useEffect(() => {
    const getSeatData = async () => {
      setIsLoading(true);
      seterror("");
      const res = await request({ url: "/api/seat/layout", method: "GET" });
      console.log(res);
      if (res.success) {
